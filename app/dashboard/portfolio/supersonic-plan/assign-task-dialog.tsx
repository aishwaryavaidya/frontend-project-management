...
export function AssignTaskDialog({
  ...
  const employeeAvatarData = employees.map((employee) => ({
    id: employee.id,
    name: employee.name,
    designation: employee.role,
    image: `https://ui.shadcn.com/assets/avatars/avatar-${employee.id}.jpg`,
  }));
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {assignments.map((assignment, index) => (
            <div key={index} className="flex items-center space-x-2">
              <AnimatedTooltip items={employeeAvatarData} />
              <Input
                type="number"
                min="1"
                max="100"
