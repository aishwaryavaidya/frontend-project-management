...
import DeleteDialog from '@/app/dashboard/project-plan/components/DeleteDialog';
import { updateTaskLevelsForIndent, updateTaskLevelsForOutdent, validatePredecessors, validateTaskDates, getMaxPredecessorEndDate, calculateSchedulePercentage } from '@/app/dashboard/project-plan/lib/taskUtils';
...
import ResourceCell from '@/app/dashboard/project-plan/components/ResourceCell';
import { 
  Select,
  SelectContent,
  SelectItem,
...
import RemarksModal from '@/app/dashboard/project-plan/components/modals/RemarksModal';
...
import { createTemplate, saveTemplate, loadTemplatesFromStorage, convertTemplateTasks, Template } from '@/app/dashboard/project-plan/lib/templateUtils';
import SaveTemplateModal from '@/app/dashboard/project-plan/components/modals/SaveTemplateModal';
import * as dateUtils from '@/app/dashboard/project-plan/lib/dateUtils';
...
export default function TaskTable({ initialTasks }: TaskTableProps) {
  ...
  const getTaskStatus = (task: Task): 'Not Started' | 'In Progress' | 'Completed' | 'Delayed' => {
    ...
    if (task.actualStartDate) return task.endDate && new Date() > task.endDate ? 'Delayed' : 'In Progress';
    return 'Not Started';
  };
  ...
  const getScheduleStatus = (schedulePercentage: number, actualProgress: number): {
    ...
    if (schedulePercentage === 0 && actualProgress === 0) {
      return {
        color: "bg-blue-500",
        status: "Yet to Start"
      };
  ...
  const renderTaskRow = (task: Task) => {
    ...
    const { color: statusColor, status } = getScheduleStatus(schedulePercentage, task.progress || 0);
        ...
        <td className="border border-gray-200 w-[100px]">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="px-2 py-1 text-xs">
                  <div className="flex items-center">
                    <div className="w-full bg-gray-200 rounded-full h-1.5 mr-2">
                      {(() => {
                        const { color } = getScheduleStatus(schedulePercentage, task.progress || 0);
                        return (
                          <div 
                            className={cn(
                              "h-1.5 rounded-full",
              ...
              <TooltipContent>
                {(() => {
                  const { status } = getScheduleStatus(schedulePercentage, task.progress || 0);
                  return (
          ...
          <div className="flex items-center">
            <span 
              className={cn(
                ...
                statusColor === "green" && "text-green-700 bg-green-100",
                statusColor === "yellow" && "text-yellow-700 bg-yellow-100",
                statusColor === "red" && "text-red-700 bg-red-100"
              )}
            >
              {status}
            </span>
        ...
        <table className="w-full border-collapse table-fixed min-w-[1200px]">
          <thead className="sticky top-0 bg-white z-20">
            <tr className="bg-gray-50">
              ...
              {/* <th className="border border-gray-200 py-1 font-medium text-gray-700 text-xs w-[80px]" title="Task Status">
                    Status
                    </th> */}
